object frmPrincipal: TfrmPrincipal
  Left = 517
  Top = 281
  Width = 800
  Height = 432
  BorderIcons = []
  BorderStyle = bsSizeToolWin
  BorderWidth = 10
  Caption = 'FI Sistemas - Chamados [ Acesso Principal ]'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Menu = MainMenu1
  OldCreateOrder = False
  Position = poMainFormCenter
  PixelsPerInch = 96
  TextHeight = 13
  object GroupBox1: TGroupBox
    Left = 0
    Top = 0
    Width = 764
    Height = 354
    Align = alClient
    TabOrder = 0
    object Image1: TImage
      Left = 8
      Top = 16
      Width = 81
      Height = 89
      Picture.Data = {
        
        800A28A2800A28A2800A28A2800A28A2803FFFD9}
      Proportional = True
    end
    object Label1: TLabel
      Left = 96
      Top = 16
      Width = 54
      Height = 13
      Caption = 'ACESSO:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label2: TLabel
      Left = 96
      Top = 40
      Width = 140
      Height = 13
      Caption = 'DATA / HORA ACESSO:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label3: TLabel
      Left = 96
      Top = 64
      Width = 87
      Height = 13
      Caption = 'TIPO ACESSO:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbNomeAtendente: TLabel
      Left = 240
      Top = 16
      Width = 54
      Height = 13
      Caption = 'ACESSO:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDataHora: TLabel
      Left = 240
      Top = 40
      Width = 87
      Height = 13
      Caption = 'DATA / HORA:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbTipoAcesso: TLabel
      Left = 240
      Top = 64
      Width = 87
      Height = 13
      Caption = 'TIPO ACESSO:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label4: TLabel
      Left = 96
      Top = 88
      Width = 123
      Height = 13
      Caption = 'DATA/HORA ATUAL:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbDataHoraAtual: TLabel
      Left = 240
      Top = 88
      Width = 87
      Height = 13
      Caption = 'TIPO ACESSO:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label5: TLabel
      Left = 8
      Top = 100
      Width = 745
      Height = 13
      Caption = 
        '----------------------------------------------------------------' +
        '----------------------------------------------------------------' +
        '----------------------------------------------------------'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object MainMenu1: TMainMenu
    Left = 592
    Top = 48
    object Cadastros1: TMenuItem
      Caption = 'Cadastros'
      object NovoCliente1: TMenuItem
        Caption = 'Novo Cliente'
        ShortCut = 114
        OnClick = NovoCliente1Click
      end
    end
    object Kanban1: TMenuItem
      Caption = 'Kanban'
      object NovaTask1: TMenuItem
        Caption = 'Novo Cadastro'
        ShortCut = 112
        OnClick = NovaTask1Click
      end
    end
    object Relatrios1: TMenuItem
      Caption = 'Relat'#243'rios'
      object FechamentoClientes1: TMenuItem
        Caption = 'Fechamento Clientes'
        ShortCut = 16496
        OnClick = FechamentoClientes1Click
      end
      object ParcialClientes1: TMenuItem
        Caption = 'Parcial Clientes'
        ShortCut = 16497
        OnClick = ParcialClientes1Click
      end
      object N1: TMenuItem
        Caption = '-'
      end
      object AtualizarAssuntosChamdos1: TMenuItem
        Caption = 'Atualizar Assuntos Chamados'
        OnClick = AtualizarAssuntosChamdos1Click
      end
      object ParcialClientesHoras1: TMenuItem
        Caption = 'Atualizar Tabela Chamados FI'
        OnClick = ParcialClientesHoras1Click
      end
      object N2: TMenuItem
        Caption = '-'
      end
      object RelatrioChamadosFISistemas1: TMenuItem
        Caption = 'Relat'#243'rio Chamados FI Sistemas [ Anual ]'
        OnClick = RelatrioChamadosFISistemas1Click
      end
      object RelatrioChamadosFISistemasPerodo1: TMenuItem
        Caption = 'Relat'#243'rio Chamados FI Sistemas [ Per'#237'odo ]'
        OnClick = RelatrioChamadosFISistemasPerodo1Click
      end
      object N3: TMenuItem
        Caption = '-'
      end
      object OTRSRelatrioAbertoGeral1: TMenuItem
        Caption = 'OTRS - Carregar Dados Relat'#243'rios'
        OnClick = OTRSRelatrioAbertoGeral1Click
      end
      object OTRSRelatrioAbertoGeral2: TMenuItem
        Caption = 'OTRS - Relat'#243'rio Aberto [ Geral ]'
        OnClick = OTRSRelatrioAbertoGeral2Click
      end
      object OTRSRelatrioAbertoPorFilaGeral1: TMenuItem
        Caption = 'OTRS - Relat'#243'rio Aberto Por Fila [ Geral ]'
        OnClick = OTRSRelatrioAbertoPorFilaGeral1Click
      end
      object OTRSRelatrioQuantidadedeChamadosPorAnalista1: TMenuItem
        Caption = 'OTRS - Relat'#243'rio Quantidade de Chamados Por Analista'
        OnClick = OTRSRelatrioQuantidadedeChamadosPorAnalista1Click
      end
      object OTRSRelatrioQuantidadedeChamadosPorAnalistaDetalhado1: TMenuItem
        Caption = 'OTRS - Relat'#243'rio Quantidade de Chamados Por Analista Detalhado'
        OnClick = OTRSRelatrioQuantidadedeChamadosPorAnalistaDetalhado1Click
      end
    end
    object Supervisor1: TMenuItem
      Caption = 'Supervisor'
    end
    object Sair1: TMenuItem
      Caption = 'Sair'
      OnClick = Sair1Click
    end
  end
  object Timer1: TTimer
    OnTimer = Timer1Timer
    Left = 632
    Top = 48
  end
end
